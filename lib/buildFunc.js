...
export default class BuildFunc {
    ...
    postHtmlProcessor(file, packageName, reload = false) {
        const quiet = true
        try {
            const sourceFile = path.resolve(path.join('src', packageName, 'html', file))
            const destFile = path.resolve(path.join('packages', packageName, 'Resources/Private', file))
            ...
            const result = this.posthtml()
                .use(this.posthtmlScssToFile({
                    'destPath': './packages/' + packageName + '/Resources/Public/Css',
